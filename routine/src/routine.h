...
#undef UINT8_ERROR
#undef SIZE_T_ERROR
#undef ULONGLONG_ERROR

#define INT_ERROR (-1)
#define UINT32_MAX (0xFFFFFFFFU)
#define SHORT_MAX (0x7FFF)
#define USHORT_MAX (0xFFFF)
#define UINT8_ERROR (0xFFFU)
#define ULONGLONG_ERROR (0xFFFFFFFFFFFFFFFFU)
...
#define SIZE_T_ERROR (0xFFFFFFFFUL)
#endif // _WIN64
#endif // __clang__
...
#endif
#ifdef STATUS_CONTROL_STACK_VIOLATION
...
#endif
#ifdef STATUS_INTERRUPTED
#undef STATUS_INTERRUPTED
#endif
#ifdef STATUS_THREAD_NOT_RUNNING
...
#error Windows 10.0.26100.0 SDK is required!
#endif // NTDDI_VERSION
...
#if defined(_M_IX86)
#define HAS_INTRINSICS (USER_SHARED_DATA->ProcessorFeatures[PF_XMMI64_INSTRUCTIONS_AVAILABLE]) // sse2
#else
#define HAS_INTRINSICS TRUE
#endif // _M_IX86
...
#define SAFE_DELETE_REFERENCE(p) {if(p) {_r_obj_clearreference ((PVOID_PTR)&(p));}}
#endif
...
FORCEINLINE R_INT128 _r_intrinsics_setint128by16 (
	_In_ SHORT value
)
{
#if defined(_ARM64_)
...
{
	return _r_str_getlength_ex ((LPWSTR)string, PR_SIZE_MAX_STRING_LENGTH);
}
...
FORCEINLINE BOOLEAN _InterlockedBitTestAndSetPointer (
	_Inout_ _Interlocked_operand_ PLONG_PTR volatile base,
	_In_ LONG_PTR bit
)
{
#if defined(_WIN64)
	return _interlockedbittestandset64 ((volatile PLONG64)base, (LONG64)bit);
#else
	return _interlockedbittestandset ((volatile PLONG)base, (LONG)bit);
#endif // _WIN64
}

FORCEINLINE LONG_PTR InterlockedExchangeAddPointer (
	_Inout_ _Interlocked_operand_ PLONG_PTR volatile addend,
	_In_ LONG_PTR value
)
{
#if defined(_WIN64)
	return (LONG_PTR)_InterlockedExchangeAdd64 ((volatile PLONG64)addend, (LONG64)value);
#else
	return (LONG_PTR)_InterlockedExchangeAdd ((volatile PLONG)addend, (LONG)value);
#endif // _WIN64
}
...
{
#if defined(_WIN64)
	return (LONG_PTR)_InterlockedIncrement64 ((volatile PLONG64)addend);
#else
	return (LONG_PTR)_InterlockedIncrement ((volatile PLONG)addend);
#endif // _WIN64
}
...
{
#if defined(_WIN64)
	return (LONG_PTR)_InterlockedDecrement64 ((volatile PLONG_PTR)addend);
#else
	return (LONG_PTR)_InterlockedDecrement ((volatile PLONG_PTR)addend);
#endif // _WIN64
}
...
)
{
	...
	value = InterlockedExchangeAddPointer ((volatile PLONG_PTR)&event_object->value, -PR_EVENT_REFCOUNT_INC);
...
{
	InterlockedExchangeAddPointer ((volatile PLONG_PTR)&event_object->value, PR_EVENT_REFCOUNT_INC);
}
...
{
	return (BOOLEAN)event_object->is_set;
}
...
)
{
	...
	if (_InterlockedBitTestAndSetPointer ((volatile PLONG_PTR)&queued_lock->value, PR_QUEUED_LOCK_OWNED_SHIFT))
...
{
	if ((LONG_PTR)(PLONG_PTR)_InterlockedCompareExchangePointer ((volatile PVOID_PTR)&queued_lock->value, LongToPtr (PR_QUEUED_LOCK_OWNED | PR_QUEUED_LOCK_SHARED_INC), NULL) != 0)
...
)
{
	...
	value = (LONG_PTR)InterlockedExchangeAddPointer ((volatile PLONG_PTR)&queued_lock->value, -(LONG_PTR)PR_QUEUED_LOCK_OWNED);
...
)
{
	...
	if ((LONG_PTR)(PLONG_PTR)_InterlockedCompareExchangePointer ((volatile PVOID_PTR)&queued_lock->value, NULL, (PVOID)value) != value)
...
)
{
	...
	// Need two memory barriers because we don't want the compiler re-ordering the following check in either direction.
	MemoryBarrier ();
...
)
{
	...
	value = ReadLongPtrAcquire (&protection->value) & ~PR_RUNDOWN_ACTIVE; // fail fast path when rundown is active

	if ((LONG_PTR)_InterlockedCompareExchangePointer ((PVOID_PTR)&protection->value, (PVOID)(value + PR_RUNDOWN_REF_INC), (PVOID)value) == value)
	{
		return TRUE;
	}
	else
...
)
{
	...
	value = (LONG_PTR)_InterlockedCompareExchangePointer ((PVOID_PTR)&protection->value, LongToPtr (PR_RUNDOWN_ACTIVE), NULL);
...
PR_WORKQUEUE_ITEM _r_workqueue_createitem (
	_In_ PR_THREAD_FUNCTION base_address,
...
VOID NTAPI _r_workqueue_queueitem (
	_Inout_ PR_WORKQUEUE work_queue,
	_In_ PR_THREAD_FUNCTION function_address,
	_In_opt_ PVOID context
);
...
VOID NTAPI _r_mem_free (
	_Frees_ptr_opt_ _Post_invalid_ PVOID base_address
);
...
// required!
FORCEINLINE ULONG_PTR _r_str_getbytelength (
	_In_ LPCSTR string
)
{
...
{
	_r_obj_initializebyteref_ex (string, (LPSTR)buffer->buffer, buffer->length);
}
...
FORCEINLINE VOID _r_obj_initializestorage (
...
FORCEINLINE VOID _r_obj_deletestringbuilder (
	_Inout_ PR_STRINGBUILDER sb
)
{
	...
	if (sb->string)
		_r_obj_clearreference ((PVOID_PTR)&sb->string);
}
...
FORCEINLINE VOID _r_obj_insertstringbuilder (
	_Inout_ PR_STRINGBUILDER sb,
	_In_ ULONG_PTR index,
	_In_ LPCWSTR string
)
...
FORCEINLINE VOID _r_obj_insertstringbuilder2 (
	_Inout_ PR_STRINGBUILDER sb,
	_In_ ULONG_PTR index,
	_In_ PR_STRINGREF string
)
...
FORCEINLINE VOID _r_obj_insertstringbuilder3 (
	_Inout_ PR_STRINGBUILDER sb,
	_In_ ULONG_PTR index,
	_In_ PUNICODE_STRING string
)
...
VOID _r_format_internetspeed (
	_Out_writes_ (buffer_length) LPWSTR buffer,
	_In_ ULONG buffer_length,
	_In_opt_ LPCWSTR locale_string,
	_In_ ULONG64 total_written,
	_In_ PLARGE_INTEGER time_start
);
...
#define PR_CALC_PERCENTOF(value, total_length)(((DOUBLE)(value) / (DOUBLE)total_length) * 100.0)
...
)
{
	...
	{
		timeout->QuadPart = -(LONG64)Int32x32To64 (milliseconds, 10000);
	}
...
FORCEINLINE ULONG _r_calc_multipledivide2 (
	_In_ CONST ULONG number,
	_In_ CONST ULONG numerator,
	_In_ CONST ULONG denominator
)
{
	return (ULONG)(((ULONG64)number * (ULONG64)numerator + (ULONG64)denominator / 2ULL) / (ULONG64)denominator);
}
...
)
{
	if (number >= 0)
	{
		return (LONG)_r_calc_multipledivide2 (number, numerator, denominator);
	}
	else
	{
		return -(LONG)_r_calc_multipledivide2 (-number, numerator, denominator);
...
_Success_ (NT_SUCCESS (return))
NTSTATUS _r_fs_createhardlink (
	_In_ PR_STRINGREF src_path,
	_In_ PR_STRINGREF dst_path
);
...
_Success_ (NT_SUCCESS (return))
NTSTATUS _r_fs_getvolumemountpoints (
	_In_ HANDLE hdrive,
	_Out_ PMOUNTMGR_MOUNT_POINTS_PTR out_buffer
);
...
BOOLEAN _r_path_getpathinfo (
	_In_ PR_STRINGREF path,
	_Out_opt_ PR_STRINGREF out_directory,
	_Out_opt_ PR_STRINGREF out_basename
);
...
NTSTATUS _r_path_search (
	_In_opt_ PR_STRINGREF search_path,
	_In_ PR_STRINGREF filename,
...
FORCEINLINE LONG _r_str_compare_logical (
	_In_ LPCWSTR string1,
	_In_ LPCWSTR string2
)
{
...
FORCEINLINE ULONG_PTR _r_str_getbytelength2 (
	_In_ PR_BYTEREF string
)
{
...
FORCEINLINE BOOLEAN _r_str_isendsswith2 (
	_In_ PR_STRINGREF string,
	_In_ LPWSTR suffix,
	_In_ BOOLEAN is_ignorecase
)
...
FORCEINLINE BOOLEAN _r_str_isstartswith2 (
	_In_ PR_STRINGREF string,
	_In_ LPWSTR prefix,
	_In_ BOOLEAN is_ignorecase
)
...
FORCEINLINE VOID _r_str_setlength (
	_Inout_ PR_STRINGREF string,
	_In_ ULONG_PTR new_length
)
{
	_r_str_setlength_ex (string, new_length, string->length);
}

FORCEINLINE VOID _r_str_skiplength (
	_Inout_ PR_STRINGREF string,
	_In_ ULONG_PTR length
)
{
...
FORCEINLINE VOID _r_str_writenullterminator (
	_In_ PR_STRINGREF string
)
{
	assert (!(string->length & 0x01));
...
FORCEINLINE VOID _r_str_trimtonullterminator (
	_In_ PR_STRINGREF string
)
{
...
PR_STRINGREF _r_sys_getcommandline ();

PR_STRINGREF _r_sys_getcurrentdirectory ();

PR_TOKEN_ATTRIBUTES _r_sys_getcurrenttoken ();

PR_STRINGREF _r_sys_getimagepath ();
...
_Success_ (NT_SUCCESS (return))
FORCEINLINE NTSTATUS _r_sys_loadlibrary2 (
	_In_ LPWSTR lib_name,
	_In_opt_ ULONG lib_flags,
	_Out_ PVOID_PTR out_buffer
)
{
	...
	return _r_sys_loadlibrary (&sr, lib_flags, out_buffer);
}
...
{
	RtlZeroMemory (environment, sizeof (R_ENVIRONMENT));
...
_Ret_maybenull_
PVOID _r_dc_getuacshield (
	_In_ LONG dpi_value,
	_In_opt_ LONG width,
	_In_opt_ LONG height,
	_In_ BOOLEAN is_icon
);
...
VOID _r_wnd_changemessagefilter (
	_In_ HWND hwnd,
	_In_reads_ (count) PULONG messages,
	_In_ ULONG_PTR count,
	_In_ ULONG action
);
...
LONG CALLBACK _r_wnd_message_callback (
	_In_ HWND hmain_wnd,
...
LONG_PTR _r_wnd_sendmessage (
	_In_ HWND hwnd,
	_In_opt_ LONG ctrl_id,
	_In_ ULONG msg,
...
{
	return (WinHttpCloseHandle (hinet) != 0);
}

FORCEINLINE BOOLEAN _r_inet_setoption (
	_In_ HINTERNET hsession,
	_In_ ULONG option,
	_In_ ULONG value
)
{
	return (WinHttpSetOption (hsession, option, &value, sizeof (ULONG)) != 0);
}
...
_Must_inspect_result_
FORCEINLINE BOOLEAN _r_math_long64tolong (
	_In_ LONG64 value,
	_Out_ _Deref_out_range_ (== , value) PLONG out_buffer)
{
	if (value >= LONG_MIN && value <= LONG_MAX)
	{
		*out_buffer = (LONG)value;
...
_Must_inspect_result_
FORCEINLINE BOOLEAN _r_math_long64toulong (
	_In_ LONG64 value,
	_Out_ _Deref_out_range_ (== , value) PULONG out_buffer)
{
	if (value >= 0 && value <= ULONG_MAX)
	{
		*out_buffer = (ULONG)value;
...
_Must_inspect_result_
FORCEINLINE BOOLEAN _r_math_long64toulong64 (
	_In_ LONG64 value,
	_Out_ _Deref_out_range_ (== , value) PULONG64 out_buffer)
{
	if (value >= 0)
	{
		*out_buffer = (ULONG64)value;

		return TRUE;
	}

	*out_buffer = ULONGLONG_ERROR;
...
BOOLEAN _r_ctrl_setbuttonmargins (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ LONG dpi_value
);
...
BOOLEAN _r_ctrl_showballoontip (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_opt_ INT icon_id,
	_In_opt_ LPCWSTR title,
	_In_opt_ LPCWSTR string
);

BOOLEAN _r_ctrl_showballoontipformat (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ INT icon_id,
	_In_opt_ LPCWSTR title,
...
FORCEINLINE BOOLEAN _r_ctrl_getidealbuttonsize (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_Out_ LPSIZE out_buffer
)
{
	RtlZeroMemory (out_buffer, sizeof (SIZE));

	return (_r_wnd_sendmessage (hwnd, ctrl_id, BCM_GETIDEALSIZE, 0, (LPARAM)out_buffer) != 0);
}
...
)
{
	...
	return (_r_wnd_sendmessage (hwnd, ctrl_id, BCM_GETTEXTMARGIN, 0, (LPARAM)out_buffer) != 0);
}
...
)
{
	...
	return (_r_wnd_sendmessage (hwnd, ctrl_id, BCM_GETIMAGELIST, 0, (LPARAM)out_buffer) != 0);
}
...
{
	return (_r_wnd_sendmessage (hwnd, ctrl_id, BCM_GETSPLITINFO, 0, (LPARAM)out_buffer) != 0);
}
...
{
	return ((_r_wnd_sendmessage (hwnd, ctrl_id, BM_GETCHECK, 0, 0) & BST_CHECKED) == BST_CHECKED);
}

FORCEINLINE VOID _r_ctrl_sendcommand (
	_In_ HWND hwnd,
	_In_ ULONG command_id,
	_In_opt_ LPARAM lparam
)
{
	_r_wnd_sendmessage (hwnd, 0, WM_COMMAND, MAKEWPARAM (command_id, 0), lparam);
}

FORCEINLINE BOOLEAN _r_ctrl_setacceleration (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ ULONG val
)
{
	...
	return (_r_wnd_sendmessage (hwnd, ctrl_id, UDM_SETACCEL, 1, (LPARAM)&ud) != 0);
}
...
_Ret_maybenull_
FORCEINLINE HICON _r_ctrl_seticon (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_opt_ HICON hicon
)
{
	return (HICON)_r_wnd_sendmessage (hwnd, ctrl_id, BM_SETIMAGE, IMAGE_ICON, (LPARAM)hicon);
}

FORCEINLINE VOID _r_ctrl_setselection (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ UINT_PTR start_pos,
	_In_ LONG_PTR end_pos
)
{
	_r_wnd_sendmessage (hwnd, ctrl_id, EM_SETSEL, start_pos, end_pos);
}
...
{
	return (_r_wnd_sendmessage (hwnd, ctrl_id, EM_SETREADONLY, is_readonly, 0) != 0);
}
...
FORCEINLINE BOOLEAN _r_datetime_getrect (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
...
)
{
	...
	return (_r_wnd_sendmessage (hwnd, ctrl_id, MCM_GETMINREQRECT, 0, (LPARAM)out_buffer) != 0);
}

_Success_ (return == GDT_VALID)
FORCEINLINE LONG_PTR _r_datetime_gettime (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_Out_ PSYSTEMTIME out_buffer
)
{
	RtlZeroMemory (out_buffer, sizeof (SYSTEMTIME));

	return _r_wnd_sendmessage (hwnd, ctrl_id, DTM_GETSYSTEMTIME, 0, (LPARAM)out_buffer);
}
...
FORCEINLINE LONG_PTR _r_datetime_setformat (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ LPCWSTR format
)
...
FORCEINLINE LONG_PTR _r_datetime_settime (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ const LPSYSTEMTIME system_time
)
...
FORCEINLINE BOOLEAN _r_edit_setcuebanner (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ LPCWSTR string
)
{
	return (_r_wnd_sendmessage (hwnd, ctrl_id, EM_SETCUEBANNER, FALSE, (LPARAM)string) != 0);
}
...
BOOLEAN _r_menu_additem_ex (
	_In_ HMENU hmenu,
	_In_opt_ ULONG item_id,
	_In_opt_ LPWSTR string,
	_In_opt_ ULONG state
);

BOOLEAN _r_menu_addsubmenu (
	_In_ HMENU hmenu,
	_In_ ULONG pos,
	_In_ HMENU hsubmenu,
	_In_opt_ LPWSTR string
...
VOID _r_menu_checkitem (
	_In_ HMENU hmenu,
	_In_ ULONG item_id_start,
	_In_opt_ ULONG item_id_end,
	_In_ ULONG position_flag,
	_In_ ULONG check_id
);
...
FORCEINLINE BOOLEAN _r_menu_additem (
	_In_ HMENU hmenu,
	_In_opt_ ULONG item_id,
	_In_ LPWSTR string
)
{
	return _r_menu_additem_ex (hmenu, item_id, string, MF_UNCHECKED);
}

FORCEINLINE BOOLEAN _r_menu_addseparator (
	_In_ HMENU hmenu
)
{
	return _r_menu_additem_ex (hmenu, 0, NULL, MF_UNCHECKED);
}

FORCEINLINE BOOLEAN _r_menu_deleteitem (
	_In_ HMENU hmenu,
	_In_ ULONG item_id,
	_In_ BOOLEAN is_byposition
)
{
	return (RemoveMenu (hmenu, item_id, is_byposition ? MF_BYPOSITION : MF_BYCOMMAND) != 0);
}
...
{
	return EnableMenuItem (hmenu, item_id, (is_byposition ? MF_BYPOSITION : MF_BYCOMMAND) | (is_enable ? MF_ENABLED : MF_DISABLED | MF_GRAYED)) != INT_ERROR;
}
...
{
	return (LONG)_r_wnd_sendmessage (hwnd, ctrl_id, HKM_GETHOTKEY, 0, 0);
}
...
FORCEINLINE VOID _r_updown_setrange (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ UINT_PTR min_value,
	_In_ LONG_PTR max_value
)
{
	_r_wnd_sendmessage (hwnd, ctrl_id, UDM_SETRANGE32, min_value, max_value);
}

FORCEINLINE LONG_PTR _r_updown_setvalue (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ LONG_PTR value
)
{
	return _r_wnd_sendmessage (hwnd, ctrl_id, UDM_SETPOS32, 0, value);
}
...
_Success_ (return != INT_ERROR)
INT _r_tab_selectitem (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ INT item_id
);

FORCEINLINE VOID _r_tab_adjustrect (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_Inout_ LPRECT out_buffer,
	_In_ BOOLEAN is_calculate
)
{
	_r_wnd_sendmessage (hwnd, ctrl_id, TCM_ADJUSTRECT, is_calculate, (LPARAM)out_buffer);
}
...
FORCEINLINE BOOLEAN _r_tab_deleteitem (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_opt_ INT item_id
)
{
	return (_r_wnd_sendmessage (hwnd, ctrl_id, TCM_DELETEITEM, item_id, 0) != 0);
}

FORCEINLINE BOOLEAN _r_tab_deleteallitems (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id
)
{
	return (_r_wnd_sendmessage (hwnd, ctrl_id, TCM_DELETEALLITEMS, 0, 0) != 0);
}
...
_Ret_maybenull_
FORCEINLINE HIMAGELIST _r_tab_getimagelist (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id
)
{
	return (HIMAGELIST)_r_wnd_sendmessage (hwnd, ctrl_id, TCM_GETIMAGELIST, 0, 0);
}
...
{
	RtlZeroMemory (out_rect, sizeof (RECT));

	return (_r_wnd_sendmessage (hwnd, 0, TCM_GETITEMRECT, (WPARAM)index, (LPARAM)out_rect) != 0);
}

_Ret_maybenull_
FORCEINLINE HIMAGELIST _r_tab_setimagelist (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_opt_ HIMAGELIST himglist
)
{
	return (HIMAGELIST)_r_wnd_sendmessage (hwnd, ctrl_id, TCM_SETIMAGELIST, 0, (LPARAM)himglist);
}
...
)
{
	...
	return (_r_wnd_sendmessage (hwnd, ctrl_id, LM_SETITEM, 0, (LPARAM)&lc) != 0);
}
...
{
	return (_r_wnd_sendmessage (hwnd, ctrl_id, LVM_DELETEITEM, (WPARAM)item_id, 0) != 0);
}
...
{
	return (_r_wnd_sendmessage (hwnd, ctrl_id, LVM_DELETEALLITEMS, 0, 0) != 0);
}

FORCEINLINE BOOLEAN _r_listview_ensurevisible (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ INT item_id
)
{
	return (_r_wnd_sendmessage (hwnd, ctrl_id, LVM_ENSUREVISIBLE, (WPARAM)item_id, FALSE) != 0);
}

FORCEINLINE INT _r_listview_getgroupcount (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id
)
{
	return (INT)_r_wnd_sendmessage (hwnd, ctrl_id, LVM_GETGROUPCOUNT, 0, 0);
}
...
{
	return (_r_wnd_sendmessage (hwnd, ctrl_id, LVM_ISGROUPVIEWENABLED, 0, 0) != 0);
}
...
{
	return (_r_wnd_sendmessage (hwnd, ctrl_id, LVM_ISITEMVISIBLE, (WPARAM)item_id, 0) != 0);
}
...
FORCEINLINE BOOLEAN _r_listview_scroll (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_opt_ LONG_PTR scroll_pos
)
{
	return (_r_wnd_sendmessage (hwnd, ctrl_id, LVM_SCROLL, 0, (LPARAM)scroll_pos) != 0);
}

_Ret_maybenull_
FORCEINLINE HIMAGELIST _r_listview_setimagelist (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_opt_ HIMAGELIST himglist
)
{
	_r_wnd_sendmessage (hwnd, ctrl_id, LVM_SETIMAGELIST, LVSIL_SMALL, (LPARAM)himglist);

	return (HIMAGELIST)_r_wnd_sendmessage (hwnd, ctrl_id, LVM_SETIMAGELIST, LVSIL_NORMAL, (LPARAM)himglist);
}

FORCEINLINE BOOLEAN _r_listview_setitemcheck (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ INT item_id,
	_In_ BOOLEAN is_check
...
{
	return _r_listview_setitemstate (hwnd, ctrl_id, item_id, INDEXTOSTATEIMAGEMASK (is_check ? 2 : 1), LVIS_STATEIMAGEMASK);
}

FORCEINLINE BOOLEAN _r_listview_setview (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ LONG view_type
)
{
	return (_r_wnd_sendmessage (hwnd, ctrl_id, LVM_SETVIEW, (WPARAM)view_type, 0) != 0);
}

FORCEINLINE BOOLEAN _r_listview_sort (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ PFNLVCOMPARE compare_func,
	_In_opt_ WPARAM lparam
)
{
	return (_r_wnd_sendmessage (hwnd, ctrl_id, LVM_SORTITEMSEX, lparam, (LPARAM)compare_func) != 0);
}
...
_Ret_maybenull_
FORCEINLINE HTREEITEM _r_treeview_getnextitem (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ ULONG retrieve_id,
	_In_opt_ HTREEITEM item_id
)
{
	return (HTREEITEM)_r_wnd_sendmessage (hwnd, ctrl_id, TVM_GETNEXTITEM, retrieve_id, (LPARAM)item_id);
}
...
FORCEINLINE BOOLEAN _r_treeview_deleteallitems (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id
)
{
	return (_r_wnd_sendmessage (hwnd, ctrl_id, TVM_DELETEITEM, 0, (LPARAM)TVI_ROOT) != 0);
}
...
FORCEINLINE BOOLEAN _r_treeview_selectitem (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ HTREEITEM hitem
)
{
	return (_r_wnd_sendmessage (hwnd, ctrl_id, TVM_SELECTITEM, TVGN_CARET, (LPARAM)hitem) != 0);
}

_Ret_maybenull_
FORCEINLINE HIMAGELIST _r_treeview_setimagelist (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_opt_ HIMAGELIST himglist
)
{
	return (HIMAGELIST)_r_wnd_sendmessage (hwnd, ctrl_id, TVM_SETIMAGELIST, TVSIL_NORMAL, (LPARAM)himglist);
}

FORCEINLINE BOOLEAN _r_treeview_setitemcheck (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ HTREEITEM item_id,
	_In_ BOOLEAN is_check
...
{
	return _r_treeview_setitemstate (hwnd, ctrl_id, item_id, INDEXTOSTATEIMAGEMASK (is_check ? 2 : 1), TVIS_STATEIMAGEMASK);
}
...
FORCEINLINE BOOLEAN _r_status_getborders (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ PVOID out_buffer
)
{
	return (_r_wnd_sendmessage (hwnd, ctrl_id, SB_GETBORDERS, 0, (LPARAM)out_buffer) != 0);
}
...
)
{
	...
	return (_r_wnd_sendmessage (hwnd, ctrl_id, SB_GETRECT, (WPARAM)part_id, (LPARAM)out_buffer) != 0);
}

FORCEINLINE BOOLEAN _r_status_seticon (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ LONG part_id,
	_In_opt_ HICON hicon
...
{
	return (_r_wnd_sendmessage (hwnd, ctrl_id, SB_SETICON, (WPARAM)part_id, (LPARAM)hicon) != 0);
}

FORCEINLINE BOOLEAN _r_status_setparts (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
...
{
	return (_r_wnd_sendmessage (hwnd, ctrl_id, SB_SETPARTS, (WPARAM)count, (LPARAM)parts) != 0);
}

FORCEINLINE BOOLEAN _r_status_settext (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ LONG part_id,
	_In_opt_ LPCWSTR string
...
{
	return (_r_wnd_sendmessage (hwnd, ctrl_id, SB_SETTEXT, MAKEWPARAM (part_id, 0), (LPARAM)string) != 0);
}
...
FORCEINLINE ULONG _r_rebar_getcount (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id
)
{
	return (ULONG)_r_wnd_sendmessage (hwnd, ctrl_id, RB_GETBANDCOUNT, 0, 0);
}
...
FORCEINLINE UINT _r_rebar_getindex (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ ULONG band_id
)
{
	return (UINT)_r_wnd_sendmessage (hwnd, ctrl_id, RB_IDTOINDEX, (WPARAM)band_id, 0);
}
...
FORCEINLINE BOOLEAN _r_rebar_getinfo (
	...
	_In_opt_ INT ctrl_id,
	_In_ ULONG band_id,
	_Inout_ LPREBARBANDINFOW out_buffer
)
{
	return (_r_wnd_sendmessage (hwnd, ctrl_id, RB_GETBANDINFO, (WPARAM)band_id, (LPARAM)out_buffer) != 0);
}

FORCEINLINE BOOLEAN _r_rebar_isbandexists (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ ULONG band_id
)
{
	return (_r_rebar_getindex (hwnd, ctrl_id, band_id) != UINT_MAX) ? TRUE : FALSE;
}

FORCEINLINE BOOLEAN _r_rebar_setinfo (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ ULONG band_id,
...
{
	return (_r_wnd_sendmessage (hwnd, ctrl_id, RB_SETBANDINFO, (WPARAM)band_id, (LPARAM)band_info) != 0);
}
...
FORCEINLINE BOOLEAN _r_toolbar_addseparator (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id
)
{
	return _r_toolbar_addbutton (hwnd, ctrl_id, 0, BTNS_SEP, NULL, 0, I_IMAGENONE);
}

FORCEINLINE BOOLEAN _r_toolbar_enablebutton (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ UINT_PTR command_id,
	_In_ BOOLEAN is_enable
...
{
	return (_r_wnd_sendmessage (hwnd, ctrl_id, TB_ENABLEBUTTON, command_id, MAKELPARAM (is_enable, 0)) != 0);
}
...
)
{
	...
	return (_r_wnd_sendmessage (hwnd, ctrl_id, TB_GETIDEALSIZE, is_ideal, (LPARAM)out_buffer) != 0);
}

FORCEINLINE LONG_PTR _r_toolbar_getstyle_ex (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id
)
{
...
_Success_ (return != INT_ERROR)
FORCEINLINE LONG _r_toolbar_getinfo (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ UINT_PTR command_id,
...
{
	RtlZeroMemory (out_buffer, sizeof (TBBUTTONINFOW));

	return (LONG)_r_wnd_sendmessage (hwnd, ctrl_id, TB_GETBUTTONINFO, command_id, (LPARAM)out_buffer);
}
...
)
{
	*out_buffer = UNICODE_NULL;
	...
	return (_r_wnd_sendmessage (hwnd, ctrl_id, TB_GETBUTTONTEXT, (WPARAM)item_id, (LPARAM)out_buffer) != 0);
}
...
{
	return (_r_wnd_sendmessage (hwnd, ctrl_id, TB_GETHOTITEM, 0, 0) == _r_wnd_sendmessage (hwnd, ctrl_id, TB_COMMANDTOINDEX, (WPARAM)command_id, 0));
}
...
{
	return (_r_wnd_sendmessage (hwnd, ctrl_id, TB_PRESSBUTTON, command_id, MAKELPARAM (is_enabled, 0)) != 0);
}
...
FORCEINLINE BOOLEAN _r_toolbar_setbuttonsize (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ ULONG width,
	_In_ ULONG height
...
{
	return (_r_wnd_sendmessage (hwnd, ctrl_id, TB_SETBUTTONSIZE, 0, MAKELPARAM (width, height)) != 0);
}
...
_Ret_maybenull_
FORCEINLINE HIMAGELIST _r_toolbar_setimagelist (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_opt_ HIMAGELIST himglist
)
{
	return (HIMAGELIST)_r_wnd_sendmessage (hwnd, ctrl_id, TB_SETIMAGELIST, 0, (LPARAM)himglist);
}
...
_Success_ (return != CLR_DEFAULT)
FORCEINLINE COLORREF _r_progress_setbarcolor (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ COLORREF clr
)
{
	return (COLORREF)_r_wnd_sendmessage (hwnd, ctrl_id, PBM_SETBARCOLOR, 0, clr);
}

_Success_ (return != CLR_DEFAULT)
FORCEINLINE COLORREF _r_progress_setbkcolor (
	_In_ HWND hwnd,
	_In_opt_ INT ctrl_id,
	_In_ COLORREF clr
)
{
	return (COLORREF)_r_wnd_sendmessage (hwnd, ctrl_id, PBM_SETBKCOLOR, 0, clr);
}
...
{
	_r_util_templatewrite (ptr, &data, sizeof (WORD));
}
...
{
	_r_util_templatewrite (ptr, &data, sizeof (ULONG));
}
