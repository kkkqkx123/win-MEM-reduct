...
)
{
	...
	if (NtCurrentPeb ()->BeingDebugged)
		return EXCEPTION_CONTINUE_SEARCH; // Remove this return to debug the exception callback.
	...
	NtTerminateProcess (NtCurrentProcess (), exception_ptr->ExceptionRecord->ExceptionCode);
...
LPWSTR _r_app_getmutexname ()
{
	...
	if (_r_initonce_begin (&init_once))
	{
		...
		cached_name = _r_format_string (
			...
			_r_app_getnameshort (),
			_r_str_gethash2 (_r_sys_getimagepath (), TRUE),
			_r_str_gethash2 (_r_sys_getcommandline (), TRUE)
		);
#else
...
BOOLEAN _r_app_isportable ()
{
	...
	if (_r_initonce_begin (&init_once))
	{
		if (_r_sys_getopt (_r_sys_getcommandline ()->buffer, L"portable", NULL))
		{
			is_portable = TRUE;
		}
		else
...
VOID _r_app_initialize_components ()
{
	...
	_r_obj_clearreference ((PVOID_PTR)&app_global.locale.default_name);
...
VOID _r_app_initialize_mitigations ()
{
	...
	status = NtSetInformationProcess (NtCurrentProcess (), ProcessMitigationPolicy, &policy, sizeof (PROCESS_MITIGATION_POLICY_INFORMATION));
...
)
{
	...
	if (_r_sys_iswow64 () && !_r_sys_getopt (_r_sys_getcommandline ()->buffer, L"nowow64", NULL))
	{
	...
	if (cmd_callback)
	{
		if (_r_sys_getopt (_r_sys_getcommandline ()->buffer, L"help", NULL))
		{
			if (cmd_callback (CmdlineHelp))
				return FALSE;
		}

		if (_r_sys_getopt (_r_sys_getcommandline ()->buffer, L"clean", NULL))
		{
			if (cmd_callback (CmdlineClean))
				return FALSE;
		}

		if (_r_sys_getopt (_r_sys_getcommandline ()->buffer, L"install", NULL))
		{
			if (cmd_callback (CmdlineInstall))
				return FALSE;
		}

		if (_r_sys_getopt (_r_sys_getcommandline ()->buffer, L"uninstall", NULL))
		{
...
PR_STRING _r_app_getdirectory ()
{
	...
	if (_r_initonce_begin (&init_once))
	{
		path = _r_sys_getimagepath ();

		cached_path = _r_path_getbasedirectory (path);
...
PR_STRING _r_app_getproxyconfiguration ()
{
	...
	proxy_string = _r_config_getstring (L"Proxy", NULL, NULL);
...
PR_STRING _r_app_getuseragent ()
{
	...
	if (_r_initonce_begin (&init_once))
	{
		cached_agent = _r_config_getstring (L"UserAgent", NULL, NULL);
...
)
{
	...
	switch (msg)
	{
		...
		case WM_SIZE:
		{
			...
			{
				if (_r_config_getboolean (L"IsMinimizeToTray", TRUE, NULL))
		...
		case WM_SYSCOMMAND:
		{
			if (wparam == SC_CLOSE)
			{
				if (_r_config_getboolean (L"IsCloseToTray", TRUE, NULL))
				{
					ShowWindow (hwnd, SW_HIDE);
...
)
{
	...
	if (!is_windowhidden)
	{
		if (_r_config_getboolean (L"IsStartMinimized", FALSE, NULL) || _r_sys_getopt (_r_sys_getcommandline ()->buffer, L"minimized", NULL))
			is_windowhidden = TRUE;
	}
#endif // APP_HAVE_TRAY
	...
	{
		if (show_code == SW_SHOWMAXIMIZED || _r_config_getboolean (L"IsMaximized", FALSE, L"window"))
		{
			if (is_windowhidden)
			{
...
)
{
	...
	_r_str_fromlong64 (buffer, RTL_NUMBER_OF (buffer), _r_locale_getversion ());

	_r_update_addcomponent (_r_app_getname (), _r_app_getnameshort (), _r_app_getversion (), _r_app_getdirectory (), TRUE);
	_r_update_addcomponent (L"Language pack", L"language", buffer, _r_app_getlocalepath (), FALSE);
	...
	// set window title
	_r_str_printf (buffer, RTL_NUMBER_OF (buffer), L"%s %s", _r_app_getname (), _r_app_getversion ());

	_r_ctrl_setstring (hwnd, 0, buffer);

	// set window prop
	SetPropW (hwnd, _r_app_getname (), LongToPtr (0x2A));

	// set window on top
	_r_wnd_top (hwnd, _r_config_getboolean (L"AlwaysOnTop", FALSE, NULL));
	...
	icon_large = _r_config_getlong (L"CheckUpdatesPeriod", APP_UPDATE_PERIOD, NULL);

	if (icon_large > 0)
		SetTimer (hwnd, GENERAL_ID, _r_calc_hours2seconds (icon_large) * 1000, NULL);
...
BOOLEAN _r_app_runasadmin ()
{
	...
	if (_r_sys_runasadmin (_r_sys_getimagepath ()->buffer, _r_sys_getcommandline ()->buffer, _r_sys_getcurrentdirectory ()->buffer) == STATUS_SUCCESS)
	{
		NtTerminateProcess (NtCurrentProcess (), STATUS_SUCCESS);
...
)
{
	...
	status = _r_sys_createprocess (_r_sys_getimagepath ()->buffer, _r_sys_getcommandline ()->buffer, _r_sys_getcurrentdirectory ()->buffer, FALSE);
...
VOID _r_config_initialize ()
{
	...
	_r_obj_movereference ((PVOID_PTR)&app_global.config.table, config_table);
...
)
{
	...
	string = _r_config_getstring (key_name, def_value ? L"true" : L"false", section_name);
...
)
{
	...
	string = _r_config_getstring (key_name, number_string, section_name);
...
)
{
	...
	font_config = _r_config_getstring (key_name, NULL, section_name);
...
)
{
	...
	pair_config = _r_config_getstring (key_name, NULL, section_name);
...
)
{
	...
	value = _r_config_getstring (key_name, def_value, section_name);
...
)
{
	...
	hash_code = _r_str_gethash (&section_string->sr, TRUE);
...
{
	_r_config_setstring (key_name, value ? L"true" : L"false", section_name);
}
...
)
{
	...
	_r_config_setstring (key_name, value_text, section_name);
}
...
)
{
	...
	_r_config_setstring (key_name, _r_obj_getstring (string), section_name);
...
)
{
	...
	hash_code = _r_str_gethash (&section_string_full->sr, TRUE);
...
VOID _r_locale_initialize ()
{
	...
	language_name = _r_config_getstring (L"Language", NULL, NULL);

	if (language_name)
	{
		if (_r_str_isstartswith (&language_name->sr, &app_global.locale.resource_name->sr, TRUE))
		{
			_r_obj_swapreference ((PVOID_PTR)&app_global.locale.current_name, app_global.locale.resource_name);
		}
		else
		{
			_r_obj_swapreference ((PVOID_PTR)&app_global.locale.current_name, language_name);
		}
	...
	{
		_r_obj_swapreference ((PVOID_PTR)&app_global.locale.current_name, app_global.locale.default_name);
	}
	...
	_r_obj_movereference ((PVOID_PTR)&app_global.locale.available_list, locale_names);
	_r_obj_movereference ((PVOID_PTR)&app_global.locale.table, locale_table);
...
)
{
	...
	else
	{
		...
		_r_obj_swapreference ((PVOID_PTR)&app_global.locale.current_name, locale_name);
	}

	_r_config_setstring (L"Language", _r_obj_getstring (app_global.locale.current_name), NULL);
	...
	if (hwindow)
	{
		...
		_r_theme_initialize (hwindow); // HACK!
	}
}
...
)
{
	...
	for (ULONG_PTR i = 0; i < locale_count; i++)
	{
		...
		if (is_menu)
		{
			menu_index = (menu_id + (ULONG)i + 2);

			_r_menu_additem (hsubmenu, menu_index, locale_name->buffer);

			if (is_current)
				_r_menu_checkitem (hsubmenu, menu_id, menu_id + (ULONG)locale_count + 1, MF_BYCOMMAND, menu_index);
		}
		else
		{
...
)
{
	...
	if (app_global.locale.current_name)
	{
		...
		hash_code = _r_str_gethash (&hash_string->sr, TRUE);
...
)
{
	...
	if (update_component->flags & PR_UPDATE_FLAG_FILE)
	{
		...
		_r_obj_movereference ((PVOID_PTR)&update_component->current_version, update_component->new_version);
...
#pragma PR_PRINT_WARNING_UNDEFINED(IDS_UPDATE_ERROR)
	...
	if (string_table)
	{
		for (ULONG_PTR i = 0; i < _r_obj_getarraysize (update_info->components); i++)
		{
			...
			hash_code = _r_str_gethash (&update_component->short_name->sr, TRUE);
			string = _r_obj_findhashtablepointer (string_table, hash_code);

			_r_obj_movereference ((PVOID_PTR)&string_value, string);
			...
			_r_obj_movereference ((PVOID_PTR)&update_component->url, _r_obj_createstring2 (&new_url_sr));
			_r_obj_movereference ((PVOID_PTR)&update_component->new_version, _r_obj_createstring2 (&new_version_sr));

			update_component->flags |= PR_UPDATE_FLAG_AVAILABLE;

			string = _r_str_formatversion (&update_component->new_version->sr);

			_r_str_appendformat (str_updates, RTL_NUMBER_OF (str_updates), L"%s - %s\r\n", update_component->full_name->buffer, string->buffer);

			_r_obj_dereference (string);

			if ((update_component->flags & PR_UPDATE_FLAG_INSTALLER) == PR_UPDATE_FLAG_INSTALLER)
			{
			...
			else if ((update_component->flags & PR_UPDATE_FLAG_FILE) == PR_UPDATE_FLAG_FILE)
			{
				if (_r_config_getboolean (L"IsAutoinstallUpdates", TRUE, NULL))
				{
					status = _r_update_downloadupdate (update_info, update_component);
...
#pragma PR_PRINT_WARNING_UNDEFINED(IDS_UPDATE_NO)
	...
	_r_config_setlong64 (L"CheckUpdatesLast", _r_unixtime_now (), NULL);
...
{
	_r_config_setlong (L"CheckUpdatesPeriod", is_enable ? APP_UPDATE_PERIOD : 0, NULL);
}
...
)
{
	...
	update_period = _r_config_getlong (L"CheckUpdatesPeriod", APP_UPDATE_PERIOD, NULL);
	...
	{
		if ((_r_unixtime_now () - _r_config_getlong64 (L"CheckUpdatesLast", 0, NULL)) <= _r_calc_hours2seconds (update_period))
			return FALSE;
	}
...
)
{
	...
	{
		if (hparent)
			_r_show_errormessage (hparent, L"Could not create update checking thread!", status, NULL, ET_NATIVE);
...
)
{
	...
	switch (msg)
	{
		case TDN_CREATED:
		{
			...
			EnableWindow (hparent, TRUE); // don't lock parent
		...
		case TDN_BUTTON_CLICKED:
		{
			...
			if (wparam == IDYES)
			{
				...
				if (NT_SUCCESS (status))
				{
					...
					{
						_r_config_setboolean (L"IsAutoinstallUpdates", TRUE, NULL);
...
#pragma PR_PRINT_WARNING_UNDEFINED(IDS_UPDATE_DOWNLOAD)
		...
		case TDN_DIALOG_CONSTRUCTED:
		{
			...
			if (_r_sys_isosversiongreaterorequal (WINDOWS_11) && !_r_config_getboolean (L"IsWindowCornerRound", FALSE, NULL))
...
)
{
	...
	log_level = _r_config_getlong (L"ErrorLevel", LOG_LEVEL_DEBUG, NULL);
...
)
{
	...
	if (tray_guid)
	{
		if (_r_config_getboolean (L"IsErrorNotificationsEnabled", TRUE, NULL))
		{
...
)
{
	...
	if (config_key && !_r_config_getboolean (config_key, TRUE, NULL))
		return TRUE;
	...
	if (status == S_OK)
	{
		if (command_id == IDYES)
		{
			if (config_key && is_flagchecked)
				_r_config_setboolean (config_key, FALSE, NULL);
...
)
{
	...
	_r_wnd_adjustrectangletoworkingarea (NULL, &rectangle_new);
	...
	if (style & WS_MAXIMIZEBOX)
	{
		...
		{
			if (_r_config_getboolean (L"IsMaximized", FALSE, window_name))
				ShowWindow (hwnd, SW_SHOWMAXIMIZED);
		}
	}
}
...
)
{
	...
	if (style & WS_MAXIMIZEBOX)
		_r_config_setboolean (L"IsMaximized", is_maximized, window_name);
...
)
{
	...
	length = ((sizeof (DLGTEMPLATEEX) + (sizeof (WORD) * 8)) + ((sizeof (DLGITEMTEMPLATEEX) + (sizeof (WORD) * 3)) * 4)) + 128;

	buffer = _r_mem_allocate (length);
	buffer_ptr = buffer;

	if (dlg_id)
		_r_config_setlong (L"SettingsLastPage", dlg_id, NULL);
	...
	// cDlgItems
#if defined(APP_HAVE_SETTINGS_TABS)
	_r_util_templatewriteshort (&buffer_ptr, 4);
#else
	_r_util_templatewriteshort (&buffer_ptr, 3);
#endif // APP_HAVE_SETTINGS_TABS
	...
	DialogBoxIndirectParamW (_r_sys_getimagebase (), (LPCDLGTEMPLATEW)buffer, hparent, &_r_settings_wndproc, 0);
...
)
{
	switch (msg)
	{
		case WM_INITDIALOG:
		{
			...
			dlg_id = _r_config_getlong (L"SettingsLastPage", 0, NULL);
...
#pragma PR_PRINT_WARNING_UNDEFINED(IDS_SETTINGS)
			...
			hitem = _r_treeview_getnextitem (hwnd, IDC_NAV, TVGN_ROOT, NULL);
			...
			while (hitem)
			{
				...
				hitem = _r_treeview_getnextitem (hwnd, IDC_NAV, TVGN_NEXT, hitem);
			}
#endif // APP_HAVE_SETTINGS_TABS
...
#pragma PR_PRINT_WARNING_UNDEFINED(IDS_CLOSE)
		...
		case WM_DESTROY:
		{
			...
			_r_wnd_top (_r_app_gethwnd (), _r_config_getboolean (L"AlwaysOnTop", FALSE, NULL));
		...
		case WM_NOTIFY:
		{
			...
			switch (lphdr->code)
			{
				...
				case TCN_SELCHANGE:
				{
					...
					_r_config_setlong (L"SettingsLastPage", ptr_page->dlg_id, NULL);
				...
				case TVN_SELCHANGING:
				{
					...
					_r_config_setlong (L"SettingsLastPage", ptr_page_new->dlg_id, NULL);
...
)
{
	...
	if (!_r_str_isequal2 (_r_sys_getimagepath (), task_path, TRUE))
		status = SCHED_E_INVALID_TASK;
...
)
{
	...
	if (is_enable)
	{
		...
		task_path = SysAllocString (_r_sys_getimagepath ()->buffer);
...
BOOLEAN _r_skipuac_run ()
{
	...
	status = IRegisteredTask_RunEx (registered_task, params, TASK_RUN_AS_SELF | TASK_RUN_USE_SESSION_ID, USER_SHARED_DATA->ActiveConsoleId, NULL, &running_task);
...
{
	_r_wnd_removecontext (hwnd, GENERAL_ID);
...
{
	_r_config_setboolean (L"IsDarkThemeEnabled", is_enable, NULL);
...
)
{
	...
	else if (_r_str_isequal2 (&class_name->sr, HOTKEY_CLASS, TRUE))
	{
		if (_r_sys_isosversiongreaterorequal (WINDOWS_10_RS5))
			_r_theme_setdarkmode (hwnd, is_enable);

		_r_theme_initializecontext (hwnd, NULL, &_r_theme_hotkey_subclass, is_enable);
	}
...
)
{
	...
	context = _r_wnd_getcontext (hwnd, GENERAL_ID);
	...
	if (is_enable)
	{
		...
		_r_wnd_setcontext (hwnd, GENERAL_ID, context);
...
BOOLEAN _r_theme_isenabled ()
{
	...
	return _r_config_getboolean (L"IsDarkThemeEnabled", is_enabled, NULL);
}
...
)
{
	...
	{
		if (_r_config_getboolean (L"IsWindowBorderEnabled", TRUE, NULL))
...
)
{
	...
	{
		RtlZeroMemory (&head_rect, sizeof (RECT));
...
LRESULT CALLBACK _r_theme_hotkey_subclass (
	_In_ HWND hwnd,
	_In_ UINT msg,
	_In_ WPARAM wparam,
	_In_ LPARAM lparam
)
{
	...
	switch (msg)
	{
		...
		case WM_ERASEBKGND:
		{
			...
			if ((_r_wnd_getstyle (hwnd, GWL_STYLE) & WS_DISABLED) == WS_DISABLED)
			{
				_r_dc_fillrect ((HDC)wparam, &rect, WND_BACKGROUND2_CLR);
			}
			else
			{
				_r_dc_fillrect ((HDC)wparam, &rect, WND_GRAYTEXT_CLR);
			}
		...
		case WM_PRINTCLIENT:
		{
			...
			//
			//value = _r_hotkey_get (hwnd, 0);
			//
			//if ((HIBYTE (value) & HOTKEYF_CONTROL) == HOTKEYF_CONTROL)
			//{
			//	GetKeyNameTextW (MAKELPARAM (0, MapVirtualKeyW (VK_CONTROL, 0)), buffer, 64);
			//
			//	_r_obj_appendstringbuilder (&sb, buffer);
			//	_r_obj_appendstringbuilder (&sb, L" + ");
			//}
			//
			//if ((HIBYTE (value) & HOTKEYF_SHIFT) == HOTKEYF_SHIFT)
			//{
			//	GetKeyNameTextW (MAKELPARAM (0, MapVirtualKeyW (VK_SHIFT, 0)), buffer, 64);
			//
			//	_r_obj_appendstringbuilder (&sb, buffer);
			//	_r_obj_appendstringbuilder (&sb, L" + ");
			//}
			//
			//if ((HIBYTE (value) & HOTKEYF_ALT) == HOTKEYF_ALT)
			//{
			//	GetKeyNameTextW (MAKELPARAM (0, MapVirtualKeyW (VK_MENU, 0)), buffer, 64);
			//
			//	_r_obj_appendstringbuilder (&sb, buffer);
			//	_r_obj_appendstringbuilder (&sb, L" + ");
			//}
			...
			//	rect.left += 2;
			//
			//	if (((_r_wnd_getstyle (hwnd, 0) & WS_DISABLED) & WS_DISABLED) == WS_DISABLED)
			//	{
			//		clr_text = WND_GRAYTEXT_CLR;
			//	}
			//	else
			//	{
			//		clr_text = WND_TEXT_CLR;
			//	}
			//
			//	_r_str_trimstring2 (&sb.string->sr, L" + ", 0);
			//
			//	_r_dc_drawtext (NULL, hdc, &sb.string->sr, &rect, 0, 0, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOCLIP, clr_text);
			//
			//	rect.left -= 2;
			//
			//	if (!(HDC)wparam)
...
)
{
	...
	for (INT i = 0; i < count; i++)
	{
		...
		is_hot = (PtInRect (&rect, context->pt) != 0);
...
)
{
	...
	clr_text = (style & WS_DISABLED) == WS_DISABLED ? WND_GRAYTEXT_CLR : WND_TEXT_CLR;
